...
namespace chrono{
	...
	void ChCSR3Matrix::SetElement(int row_sel, int col_sel, double insval, bool overwrite)
	{
		...
		if (insval == 0 || m_lock) //TODO: do we really want to insert 0 while sparsity is locked? if so, then add || m_lock
			return;
		int trail_i;
		for (trail_i = leadIndex[lead_sel]; trail_i<leadIndex[lead_sel]; ++trail_i)
		{
	...
	double ChCSR3Matrix::GetElement(int row_sel, int col_sel)
	{
		auto lead_sel = row_major_format ? row_sel : col_sel;
		auto trail_sel = row_major_format ? col_sel : row_sel;

		for (auto trail_i = leadIndex[lead_sel]; trail_i<leadIndex[lead_sel]; ++trail_i)
		{
			if (trailIndex[trail_i] == trail_sel)
	...
	{
		auto lead_dim = row_major_format ? nrows : ncols;
		auto trail_dim = row_major_format ? ncols : nrows;

		if (nonzeros !=0 || lead_dim!=*leading_dimension || trail_dim!=*trailing_dimension || !m_lock)
		{
			reset_arrays(lead_dim, trail_dim, nonzeros); // breaks also the sparsity lock
		}
	...
	bool ChCSR3Matrix::Compress()
	{
		...
		{
			for (trail_i = leadIndex[lead_i]; trail_i<leadIndex[lead_i]; ++trail_i)
			{
				if (initialized_element[trail_i])
				{
		...
		values.resize(trail_i_dest);
		trailIndex.resize(trail_i_dest);
	...
	void ChCSR3Matrix::Prune(double pruning_threshold)
	{
		...
		for (auto lead_i = 0; lead_i<*leading_dimension; ++lead_i)
		{
			for (auto trail_i = leadIndex[lead_i]; trail_i<leadIndex[lead_i]; ++trail_i)
			{
	...
	{
		double delta = static_cast<double>(final_number - initial_number) / vector.size();
	...
	void ChCSR3Matrix::insert(int& trail_sel, const int& lead_sel)
	{
		...
		bool OK_also_onelement_rows = true;
		...
		for (auto trail_i = trail_sel - 1; trail_i >= 0, (trail_sel - trail_i) < std::min(max_shifts, shift_fw); --trail_i)
		{
			if (!initialized_element[trail_i]) // look for not initialized elements
			{
				// check if it is out of row
		...
		if (shift_bw==0 && shift_fw==0)
		{
			...
			for (lead_sel_fw = trail_sel + 1; lead_sel_fw <= *leading_dimension; ++lead_sel_fw)
			{
