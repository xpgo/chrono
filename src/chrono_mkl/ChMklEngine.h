...
/// @{
	...
	public:
		...
		*/
		...
		void SetMatrix(ChCSR3Matrix& Z); ///< It sets the matrix, the problem size \c n and the matrix type \c mtype
		...
		void SetProblem(ChCSR3Matrix& Z, ChMatrix<>& insb, ChMatrix<>& insx); ///< It sets the data arrays, the problem size \c n and the matrix type \c mtype

		// Solver routine
		int PardisoCall(int set_phase = 13, int message_level = 0); ///< Calls the Pardiso Solver
